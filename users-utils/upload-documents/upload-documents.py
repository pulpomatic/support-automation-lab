...
def get_files_from_directory(directory, user_license_number):
    """Obtiene una lista de archivos en un directorio y sus subdirectorios."""
    files = []
    for root, _, filenames in os.walk(directory):
        for filename in filenames:
            if filename.startswith(f"{user_license_number} "):
...
def process_csv(csv_path, base_dir):
    ...
    with open(csv_path) as csvfile:
        ...
        with tqdm(total=total, desc="Procesando archivos") as progress_bar:
            for row in archivo:
                user_license_number = row[0]
                user_id = row[1]

                url = f"{BASE_URL}/documents/archives/USERS/{user_id}?path="

                if os.path.isdir(base_dir):
                    files = get_files_from_directory(base_dir, user_license_number)
                    if files:
